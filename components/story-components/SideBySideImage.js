    ...
    function renderDefaultImage(description, cloudinaryId) {
      return (
        <div className="relative">
          <img
            alt={description}
            className={`attachment-fixed object-cover w-full h-full subtle-shadow rounded-sm ${
              largerImageHeight && 'lg:max-h-[300px]'
            }`}
        ...
        >
          ...
          >
            <div className={`mb-3 flex flex-col items-center`}>
              <Title
                withCSSBorder={false}
                size={`text-5xl text-secondary text-center  xl:text-5xl mt-4  w-fit     normal-case ${
            ...
            <div
              className={`text-white text-base text-center font-body story-body leading-loose mb-12 text-white max-w-lg m-auto`}
            >
          ...
          <div className={`flex justify-center`}>
            {linkTitle && linkAddress ? (
              <CTA classList="py-8" url={`${linkAddress}`}>
                {linkTitle}
              </CTA>
            ) : (
              ''
